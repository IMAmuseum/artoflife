...
def processABBYY(page):
    ...
    try:
        ...
        helper.log.debug("ABBYY for scan_id: %s" % (scan_id))
        ...
        helper.log.debug("ABBYY Complete for scan_id: %s" % (scan_id))
        return result
    except:
        helper.log.error("error processing ABBYY for scan_id %s" % (scan_id))
        return False
