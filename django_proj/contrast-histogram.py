...
if __name__ == '__main__':
    ...
    for page in pages:
        ...
        if ('contrast' in page) and ('max_contiguous' in page['contrast']):
            if page['has_illustration']['gold_standard']:
                with_v.append(page['contrast']['max_contiguous'])
            else:
                without_v.append(page['contrast']['max_contiguous'])
        else:
            print 'No contrast on', page['scan_id'], page['ia_page_num']
    ...
    ax.set_xlabel('Max contiguous ratio')
